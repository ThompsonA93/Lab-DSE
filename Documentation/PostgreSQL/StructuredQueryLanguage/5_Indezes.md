# Indexes 

- [Indexes](#indexes)
  - [Index Types in PostgreSQL](#index-types-in-postgresql)
    - [B-Tree Index](#b-tree-index)
    - [Hash Index](#hash-index)
    - [GiST Index](#gist-index)
    - [GIN Index](#gin-index)
  - [Index Creation \& Management](#index-creation--management)
    - [Simple Indexes](#simple-indexes)
    - [Composite Indexes](#composite-indexes)
    - [Partial Indexes](#partial-indexes)
    - [Viewing and Dropping Indexes](#viewing-and-dropping-indexes)
  - [Index Optimization](#index-optimization)
    - [Index Scans vs. Sequential Scans](#index-scans-vs-sequential-scans)
    - [Using `EXPLAIN`](#using-explain)
    - [Trade-Offs: Speed and Cost](#trade-offs-speed-and-cost)



## Index Types in PostgreSQL

### B-Tree Index

### Hash Index

### GiST Index

### GIN Index

## Index Creation & Management

### Simple Indexes

### Composite Indexes

### Partial Indexes

### Viewing and Dropping Indexes


## Index Optimization

### Index Scans vs. Sequential Scans

### Using `EXPLAIN`

### Trade-Offs: Speed and Cost